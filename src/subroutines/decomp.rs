...
use crate::prime_ring::ring::{DPrimeRingElement, PrimeRingElement};
...
pub fn decomp_ell(power_series: &Vec<PowerSeries>, witness: &Vec<Vec<DPrimeRingElement>>, parts: usize) -> (Vec<Vec<DPrimeRingElement>>, DecompOutput) {
    use std::time::Instant;
    // Decompose each column of the decomposition of the witness matrix
    let now = Instant::now();
