use num_traits::One;
use crate::arithmetic::{PowerSeries, sample_random_vector};
use crate::prime_ring::r#static::{MODULE_SIZE, WIT_DIM};
...
pub fn compute_commitment_keys(module: Vec<DPrimeRingElement>, wit_dim: usize) -> Vec<PowerSeries> {
    module.into_par_iter().map(|m| {
        ...
        while current_dim % 2 == 0 {
            ...
            current_dim /= 2;
            ps.tensors.push(vec![PrimeRingElement::one(), row[current_dim - 1]]);
        }
        ps.expanded_layers.push(row[0..current_dim].to_vec());
        ps
    }).collect()
