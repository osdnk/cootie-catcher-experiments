...
use crate::cpp::bindings::{ntt_forward_in_place, ntt_inverse_in_place};
use crate::prime_ring::r#static::{MOD_Q};
use crate::ntt::root_of_unity::choose_root_unity;
...
pub fn ntt_pow_of_2(a: &mut [u64], mod_q: u64) {
    #[cfg(all(target_arch = "x86_64", any(feature = "use-hardware", feature = "partial-hardware")))]
    unsafe {
        return ntt_forward_in_place(a.as_mut_ptr(),  a.len(), mod_q);
    }
...
pub fn inverse_ntt_pow_of_2(a: &mut [u64], mod_q: u64) {
    #[cfg(all(target_arch = "x86_64", any(feature = "use-hardware", feature = "partial-hardware")))]
    unsafe {
        return ntt_inverse_in_place(a.as_mut_ptr(),  a.len(), mod_q);
    }
    let root = choose_root_unity(a.len(), mod_q as i128).unwrap() as u64;
